...
def setup():
    ...
    if current_os in ('Darwin', 'Linux'):
        ...
        if count == len(project_files):
            utils.s_print('setup', 'I', '[1/3] OK: Project files already installed')
        else:
            try:
                ...
                for elem in project_files:
                    ...
                    utils.s_print('setup', 'I', f'[1/3] OK: {setup_folder}{elem}')
                utils.s_print('setup', 'I', f'[1/3] OK: Copied the project into {setup_folder}')
            except FileNotFoundError as not_found_err:
                utils.s_print('setup', 'E', f'[1/3]{not_found_err}')

        # Step 2: Add the alias into .bashrc/.zshrc
        shell = environ['SHELL']
        shell_string = shell.split('/')[2] if 'bash' in shell or 'zsh' in shell else None
        if not shell_string:
            utils.s_print('setup', 'E', '[2/3] Please use a bash or zsh shell to install the command system-wide')
        else:
            ...
            with open(f'{home}/{rc_file}', 'a') as write_rc:
                ...
                if read_rc:
                    if 'shlerp' in read_rc:
                        write = False
                        utils.s_print('setup', 'I', '[2/3] OK: Alias function already installed')
                if write:
                    write_rc.write(f'source {setup_folder}function.template')
                    utils.s_print('setup', 'I', f'[2/3] OK: Alias added to {rc_file}')
        ...
        def check_deps(first_try):
            ...
            if utils.req_installed(setup_folder):
                utils.s_print('setup', 'I', f'[3/3] OK: Virtual environment {word} installed')
            else:
                utils.s_print('setup', 'E', 'ERROR: A problem happened during the requirements installation')
                exit(0)
        ...
        utils.s_print('setup', 'E', 'ERROR: Shell or system not supported')
        exit(0)
